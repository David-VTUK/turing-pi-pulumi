...
func installK3s(ctx *pulumi.Context, nodes []node, k3sversion string) error {
	...
	for index, node := range nodes {
		if index == 0 {
			...
			k3sSubsequentNodeCommand := fmt.Sprintf("curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION=\"%s\" INSTALL_K3S_EXEC=\"agent --server https://%s:6443 --token myrandompassword --data-dir /mnt/data/k3s\" sh -s -", k3sversion, nodes[0].addresss)
			conn := remote.ConnectionArgs{
				Host:       pulumi.String(node.addresss),
				User:       pulumi.String(node.username),
				PrivateKey: pulumi.String(node.sshKey),
